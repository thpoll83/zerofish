...
_GBW = (ui.VSEP_X - 2 * _OM - (_COLS - 1) * _GAP) // _COLS
_GBH = (ui.H - ui.TITLE_H - 2 * _OM - (_ROWS - 1) * _GAP) // _ROWS
...
_THIRD    = (_TOTAL - 4) // 3
_NEXT3_Y0 = _RY0
_NEXT3_Y1 = _RY0 + _THIRD
_OK3_Y0   = _NEXT3_Y1 + 2
_OK3_Y1   = _OK3_Y0 + _THIRD
_BACK3_Y0 = _OK3_Y1 + 2
_BACK3_Y1 = _RY1

_BTN_CX = (ui.OK_X0 + ui.OK_X1) // 2
...
class ResumeScreen(ui.Screen):
    name = 'resume_games'

    def build(self, saves: list, page: int = 0,
        ...
        for slot, sv in enumerate(page_saves):
            ...
            if is_white_player:
                ...
                if active:
                    ui.draw_btn_bar_inv(draw, [(x0, y0), (x1, y1)])
                else:
            ...
            cx = (x0 + x1) // 2
            cy = (y0 + y1 - 5) // 2 - 2
            ui.draw_centered(draw, cx, cy, _btn_label(sv), f['small'],
                             text_fill, line_spacing=13)
    ...
    def hit(self, lx: int, ly: int, saves=None, sel=None) -> str | None:
        show_next = saves is not None and len(saves) > 6
        if hit_back(lx, ly, show_next):
            return 'back'
        if show_next and hit_next(lx, ly, show_next):
            return 'next_page'
        if hit_ok(lx, ly, show_next) and sel is not None:
            return 'ok'
        if saves is not None:
            page_count = min(6, len(saves))
            for slot in range(page_count):
                if hit_game_btn(slot, lx, ly):
                    return f'slot:{slot}'
        return None


_screen = ResumeScreen()


def build_resume_screen(saves: list, page: int = 0,
                        sel: int | None = None) -> Image.Image:
    return _screen.build(saves, page=page, sel=sel)
